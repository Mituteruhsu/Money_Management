...
class o_data:
    def redata(df):
        ...
        df = market_index.index_deviation(df)
        df = df.set_index('日期')
        df_2022 = df['2022']
...
class imdraw:
    def image(df):
        date = redata.date
        net_worth = redata.net_worth
        index_deviation = redata.index_deviation
        index_d_m10 = redata.index_d_m10
        index_d_p10 = redata.index_d_p10
        # 繪圖
        return (
...
print(o_data.redata.df)


# plt.figure(figsize=(18,10))  # 設定圖像大小

# plt.subplot(2, 2, 1)
# market_index.image(redata)
